	...
	if (unlikely(ret))
		 ...
		 */
                if (workingset_refault(shadow))
                 printk(KERN_CRIT"refault page_uid = %d\n", page->uid_lenny); //lenny_test
		...
		struct iov_iter *iter, ssize_t written)
{
	...
	int error = 0;
       // printk(KERN_CRIT"fgapp uid = %d\n", fgapp_uid); //lenny_test
...
readpage:
              // printk(KERN_CRIT"pid=%d,uid=%u,page uid=%u,name=%s\n", current->pid, current->cred->uid.val, page->uid_lenny, current->comm); //lenny1
		/*
