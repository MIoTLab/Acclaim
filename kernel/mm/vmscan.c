...
activate_locked:
		...
		if (PageSwapCache(page) && vm_swap_full())
			try_to_free_swap(page);
                               ...
                               if( (page->uid_lenny!=10106) && (page->uid_lenny!=10094) && (page->uid_lenny!=10112) && (page->uid_lenny!=10092) && (page->uid_lenny!=10093) && (page->uid_lenny!=10109) && (page->uid_lenny!=10098) && (page->uid_lenny!=10095)&& (page->uid_lenny!=10107) && (page->uid_lenny!=10108) && (page->uid_lenny!=10100) && (page->uid_lenny!=10102) && (page->uid_lenny!=10101) && (page->uid_lenny!=10096) && (page->uid_lenny!=10103) &&(page->uid_lenny!=10105) && (page->uid_lenny!=10097) && (page->uid_lenny!=10099) && (page->uid_lenny!=10090)) // if it is not background app as normal if it is background stay in inactive
               #endif
              {
		VM_BUG_ON_PAGE(PageActive(page), page);
		SetPageActive(page);
	...
	while (!list_empty(&l_hold)) {
		...
		if (page_referenced(page, 0, sc->target_mem_cgroup,
			...
			if ((vm_flags & VM_EXEC) && page_is_file_cache(page)) {
                            #ifdef lenny_fast_uid
                                if( (page->uid_lenny!=10106) && (page->uid_lenny!=10094) && (page->uid_lenny!=10112) && (page->uid_lenny!=10092) && (page->uid_lenny!=10093) && (page->uid_lenny!=10109) && (page->uid_lenny!=10098) && (page->uid_lenny!=10095)&& (page->uid_lenny!=10107) && (page->uid_lenny!=10108) && (page->uid_lenny!=10100) && (page->uid_lenny!=10102) && (page->uid_lenny!=10101) && (page->uid_lenny!=10096) && (page->uid_lenny!=10103) &&(page->uid_lenny!=10105) && (page->uid_lenny!=10097) && (page->uid_lenny!=10099) && (page->uid_lenny!=10090)) //if it is not a background page, it will be moved to the head of active list. if it is background, it will be go to inactive list.
                            #endif
                               {
				 ...
				 struct lruvec *lruvec, struct scan_control *sc)
{
	if (is_active_lru(lru)) {
		if (inactive_list_is_low(lruvec, lru))
                       {
                       // printk(KERN_CRIT"shrink_activelist= %d\n", lru); //lenny_test shrink_active_list
			shrink_active_list(nr_to_scan, lruvec, sc, lru);
                       }
		return 0;
	}
      //  printk(KERN_CRIT"shrink_inactivelist= %d\n", lru); //lenny_test shrink_active_list
			  ...
			  struct scan_control *sc, unsigned long *lru_pages)
{
	...
	sc->nr_reclaimed += nr_reclaimed;
        //printk(KERN_CRIT"reclaim_number= %lu\n", sc->nr_reclaimed); //lenny_fast shrink page number
	/*
	...
	if (inactive_anon_is_low(lruvec))
	{
        shrink_active_list(SWAP_CLUSTER_MAX, lruvec,
				   sc, LRU_ACTIVE_ANON);
      //  printk(KERN_CRIT"active_anon=1\n"); //lenny many dirty data need to be written back
        }
	...
	do {
		...
		if (total_scanned > writeback_threshold) {
			wakeup_flusher_threads(laptop_mode ? 0 : total_scanned,
                       ...
                       // printk(KERN_CRIT"lennyflush= %d\n", total_scanned); //lenny_fast flush
			sc->may_writepage = 1;
		}
	} while (--sc->priority >= 0);
			       ...
			       struct scan_control *sc)
{
	...
	bool lowmem_pressure;
         #ifdef lenny_fast
        unsigned long lennymax=high_wmark_pages(zone); //lenny
        int lennyratio=lenny_get_sum();//lenny
	/* Reclaim above the high watermark. */
	
        if(lennyratio<=0) {}
        else if (lennyratio>100)
        lennymax= min(lennymax+lennymax*(lennyratio-100)/100,8856);//lenny
        else
        lennymax= max(lennymax-lennymax*(100-lennyratio)/100, 8020);//lenny
        
        sc->nr_to_reclaim = max(SWAP_CLUSTER_MAX, lennymax); //lenny
         #endif
       // sc->nr_to_reclaim = max(SWAP_CLUSTER_MAX, high_wmark_pages(zone));
       // printk(KERN_CRIT" ratio= %lu predict= %d\n", lenny_get_sum(),lennymax ); //lenny_test shrink page number
	/*
	...
	for ( ; ; ) {
		...
		if (!ret) {
			...
			balanced_classzone_idx = balance_pgdat(pgdat, order,
								classzone_idx);
                        //printk(KERN_CRIT"reclaim_way=kswapd, order=%d\n", order); //lenny kswapd reclaim	
		}
	}
...
void wakeup_kswapd(struct zone *zone, int order, enum zone_type classzone_idx)
{
	...
	pgdat = zone->zone_pgdat;
        #ifdef lenny_fast
        if (lenny_get_sum()>200)  //lenny_fast1
        pgdat->kswapd_max_order=lenny_get_accuracy()+2; //lenny_fast1
        else
        pgdat->kswapd_max_order=lenny_get_accuracy()+1; //lenny_fast1
        #endif	
       // printk(KERN_CRIT"lenny_size=%d\n", pgdat->kswapd_max_order); //lenny_test new_order
