...
static int check_output_buffer_vaild(jpgenc_config_t* config)
{
	struct ion_handle *ionhnd = NULL;
	struct iommu_map_format iommu_format;
	int ret = 0;

	if ((config->buffer.ion_fd < 0) || IS_ERR_OR_NULL(s_hjpeg150.ion_client)) {
		cam_err("invalid ion: fd=%d", config->buffer.ion_fd);
		return -1;
	}

	ionhnd = ion_import_dma_buf(s_hjpeg150.ion_client, config->buffer.ion_fd);//lint !e838
	if (IS_ERR(ionhnd)) {
		ionhnd = NULL;
		return -1;//lint !e438
	} else {
		...
		if (config->buffer.output_buffer != iommu_format.iova_start) {
			ret = -1;
		}
	}
...
static int check_config(jpgenc_config_t* config)
{
    ...
    return check_output_buffer_vaild(config);
}
